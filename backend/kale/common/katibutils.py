...
from kale.common import k8sutils, kfputils, podutils, workflowutils
...
TRIAL_PARAMETERS_BASE = [{"name": KALE_PARAM_TRIAL_NAME, "reference": "${trialSpec.Name}"}]
    ...
    "spec": {
        ...
        "template": {
            ...
            "spec": {
                ...
                "containers": [{"name": TRIAL_CONTAINER_NAME, "image": TRIAL_IMAGE}],
            },
        },
    },
}
...
def create_and_wait_kfp_run(
    ...
    **kwargs,
    ...
    log.info("Annotating Trial '%s' with the KFP Run UUID '%s'...", run_name, run_id)
    try:
        # Katib Trial name == KFP Run name by design (see rpc.katib)
        annotate_trial(
            run_name, pod_namespace, {KALE_KATIB_KFP_ANNOTATION_KEY: run_id}, api_version
        )
    except Exception:
        log.exception("Failed to annotate Trial '%s' with the KFP Run UUID '%s'", run_name, run_id)

    log.info("Getting Workflow name for run '%s'...", run_id)
    workflow_name = kfputils.get_workflow_from_run(kfputils.get_run(run_id))["metadata"]["name"]
    log.info("Workflow name: %s", workflow_name)
    log.info("Getting the Katib trial...")
    trial = get_trial(run_name, pod_namespace, api_version)
    log.info("Trial name: %s, UID: %s", trial["metadata"]["name"], trial["metadata"]["uid"])
    ...
    try:
        workflowutils.annotate_workflow(workflow_name, pod_namespace, wf_annotations)
    except Exception:
        log.exception("Failed to annotate Workflow '%s' with the Katib details", workflow_name)

    status = kfputils.wait_kfp_run(run_id)

    # If run has not succeeded, return no metrics
    if status != "Succeeded":
        log.warning("KFP run did not run successfully. No metrics to return.")
...
def annotate_trial(name, namespace, annotations, api_version=KATIB_API_VERSION_V1BETA1):
    """Add annotations to a Trial."""
    k8sutils.annotate_object(
        KATIB_API_GROUP, api_version, KATIB_TRIALS_PLURAL, name, namespace, annotations
    )
...
def get_trial(name, namespace, api_version=KATIB_API_VERSION_V1BETA1):
    ...
    return k8s_client.get_namespaced_custom_object(
        KATIB_API_GROUP, api_version, namespace, KATIB_TRIALS_PLURAL, name
    )


def _get_owner_experiment(owner_references, api_version=KATIB_API_VERSION_V1BETA1):
    owner = None
    expected_api_version = f"{KATIB_API_GROUP}/{api_version}"
    for ref in owner_references:
        api_version = ref.get("apiVersion")
        kind = ref.get("kind")
        controller = ref.get("controller")
        if api_version == expected_api_version and kind == "Experiment" and controller:
            owner = ref
            break
    return owner
...
def get_owner_experiment_from_trial(trial):
    ...
    owner = _get_owner_experiment(trial["metadata"]["ownerReferences"], api_version=api_version)
    if not owner:
        trial_name = trial["metadata"]["name"]
        trial_namespace = trial["metadata"]["namespace"]
        trial_uid = trial["metadata"]["uid"]
        raise RuntimeError(
            "Failed to find owner Experiment for Trial."
            f" Name: {trial_name}, Namespace: {trial_namespace} UID: {trial_uid}"
        )
    return owner.get("name"), owner.get("uid")


def construct_experiment_cr(
    name,
    experiment_spec,
    pipeline_id,
    version_id,
...
def _construct_experiment_cr_v1alpha3(
    name, experiment_spec, pipeline_id, version_id, experiment_name
):
    ...
    experiment["spec"]["trialTemplate"] = {"goTemplate": {"rawTemplate": raw_template}}

    return experiment


def _construct_experiment_cr_v1beta1(
    name, experiment_spec, pipeline_id, version_id, experiment_name
):
    ...
    trial_tmpl["trialParameters"].extend([{"name": p, "reference": p} for p in param_names])

    kwargs = {p: f"${{trialParameters.{p}}}" for p in param_names}
    kwargs.update(
        {
            "pipeline_id": pipeline_id,
            "version_id": version_id,
            "run_name": f"${{trialParameters.{KALE_PARAM_TRIAL_NAME}}}",
            "experiment_name": experiment_name,
            "api_version": KATIB_API_VERSION_V1BETA1,
        }
    )
    cmd = JOB_CMD % ", ".join(f"{k}='{v}'" for k, v in kwargs.items())
    job = _get_base_job_cr()
    job["spec"]["template"]["spec"]["containers"][0]["command"] = [cmd]
    trial_tmpl["trialSpec"] = job
    spec["trialTemplate"] = trial_tmpl
    experiment = {
        "apiVersion": f"{KATIB_API_GROUP}/{KATIB_API_VERSION_V1BETA1}",
        "kind": "Experiment",
        "metadata": {"name": name},
        "spec": spec,
    }
...
def create_experiment(experiment, namespace):
    ...
    log.info("Creating Katib Experiment '%s/%s'...", namespace, experiment["metadata"]["name"])
    api_version = experiment["apiVersion"].split("/")[1]
    exp = k8s_co_client.create_namespaced_custom_object(
        KATIB_API_GROUP, api_version, namespace, KATIB_EXPERIMENTS_PLURAL, experiment
    )
...
def list_experiments(namespace, api_version=KATIB_API_VERSION_V1BETA1):
    ...
    k8s_co_client = k8sutils.get_co_client()
    log.info("Listing Katib %s Experiment n namespace '%s'...", api_version, namespace)
    ...
    try:
        ...
        if e.status != 403:
            ...
            try:
                ...
                if e.status != 403:
                    raise RuntimeError(
                        "Katib is not installed or has an"
