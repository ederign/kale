...
from kale.common import astutils, flakeutils, jputils, k8sutils, podutils, utils
...
API_VERSION = f"{CO_GROUP}/{CO_VERSION}"
...
TRANSFORMER_ASSETS_DIR = os.path.join(PVC_ROOT, ".kale.kfserving-transformer.dir")
...
class KFServer:
    ...
    def __repr__(self):
        ...
        if utils.is_ipython():
            ...
            html = (
                f"InferenceService <pre>{self.name}</pre> serving requests at host"
                f" <pre>{get_inference_service_host(self.name)}</pre><br>"
                f'View model <a href="/models/details/{podutils.get_namespace()}/{self.name}"'
    ...
    def delete(self):
        ...
        k8s_co_client.delete_namespaced_custom_object(
            CO_GROUP, CO_VERSION, namespace, CO_PLURAL, self.name
        )
    ...
    def predict(self, data: str, tensor=False):
        ...
        response = requests.post(
            f"http://cluster-local-gateway.istio-system/v1/models/{self.name}:predict",
            data=data,
            headers=headers,
        )
        if response.status_code == 200:
            ...
            log.error("The request failed with status code %s", response.status_code)
            return response

    def _to_tensor(self, data):
        log.warning("Kale does not yet support converting the predicted response to a tensor.")
        return data
...
def serve(
    ...
    preprocessing_assets: dict = None,
) -> KFServer:
    ...
    * Monitor the CR until it becomes ready
    ...
    if not name:
        name = f"{podutils.get_pod_name()}-{utils.random_string(5)}"
    ...
    if predictor and predictor != predictor_type:
        raise RuntimeError(
            "Trying to create an InferenceService with"
            f" predictor of type '{predictor}' but the model is of type"
            f" '{predictor_type}'"
        )
...
def _prepare_transformer_assets(fn: Callable, assets: dict = None):
    ...
    shutil.copy(notebook_path, os.path.join(TRANSFORMER_ASSETS_DIR, TRANSFORMER_SRC_NOTEBOOK_NAME))
...
def create_inference_service(
    ...
    if predictor not in PREDICTORS:
        raise ValueError(f"Invalid predictor: {predictor}. Choose one of {PREDICTORS}")

    if predictor == "custom":
        if not image:
            raise ValueError("You must specify an image when using a custom predictor.")
        if not port:
            raise ValueError("You must specify a port when using a custom predictor.")
        ...
        if image is not None:
            log.info(
                "Creating an InferenceService with predictor '%s'. Ignoring image...", predictor
            )
        if port is not None:
            log.info(
                "Creating an InferenceService with predictor '%s'. Ignoring port...", predictor
            )
        predictor_spec = PVC_PREDICTOR_TEMPLATE.format(
            predictor=predictor, pvc_name=pvc_name, model_path=model_path
        )
    ...
    if transformer:
        transformer_spec = yaml.safe_load(
            TRANSFORMER_CUSTOM_TEMPLATE.format(
                image=podutils.get_docker_base_image(), pvc_name=pvc_name, pvc_mount_point=PVC_ROOT
            )
        )
        infs_spec["spec"]["default"]["transformer"] = transformer_spec

    yaml_filename = f"{name}.kfserving.yaml"
...
def _get_runtime_version(predictor: str):
    ...
    if not library:
        raise ValueError("The provided predictor is not backed by any Kale marshalling backend.")
    if len(library) > 1:
        raise ValueError(f"Too many backends are matching the '{predictor}' predictor: {library}")
...
def _submit_inference_service(inference_service: dict, namespace: str):
    ...
    try:
        k8s_co_client.create_namespaced_custom_object(
            CO_GROUP, CO_VERSION, namespace, CO_PLURAL, inference_service
        )
    except ApiException:
...
def _add_owner_references(infs_name: str, pvc_name: str):
    ...
    log.info("Adding owner references to PVC '%s' for InferenceService '%s'", pvc_name, infs_name)
    client = k8sutils.get_v1_client()
    infs = get_inference_service(infs_name)
    pvc = client.read_namespaced_persistent_volume_claim(pvc_name, podutils.get_namespace())
    ref = kubernetes.client.V1OwnerReference(
...
def get_inference_service(name: str):
    ...
    return k8s_co_client.get_namespaced_custom_object(CO_GROUP, CO_VERSION, ns, CO_PLURAL, name)
...
def get_inference_service_default_predictor_host(name: str) -> str:
    ...
    try:
        ...
        log.error("Could not find the predictor's url for InferenceService '%s'", name)
        return ""
