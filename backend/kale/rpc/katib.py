...
from kale.common import k8sutils, katibutils, podutils
from kale.rpc.errors import RPCNotFoundError, RPCUnhandledError

KATIB_PARAMETER_NAMES = (
    "objective",
    "algorithm",
    "parallelTrialCount",
    "maxTrialCount",
...
KATIB_DEFAULTS = {"parallelTrialCount": 3, "maxTrialCount": 12, "maxFailedTrialCount": 3}
KATIB_EXPERIMENT_STATUS = ["Failed", "Succeeded", "Restarting", "Running", "Created"]
...
def _launch_katib_experiment(request, katib_experiment, namespace):
    ...
    try:
        ...
        raise RPCUnhandledError(
            message="Failed to launch Katib experiment", details=str(e), trans_id=request.trans_id
        )


def _sanitize_parameters(request, parameters, parameter_names, defaults, parameters_type):
    """Keep just the known parameter fields that are required."""
    sanitized = {}
    for param in parameter_names:
        if param not in parameters and param not in defaults:
            request.log.exception("%s parameter '%s' was not provided", parameters_type, param)
            raise RPCNotFoundError(
                details=(f"{parameters_type} parameter '{param}' is required"),
                trans_id=request.trans_id,
            )
        sanitized[param] = parameters.pop(param, defaults.get(param))
    if parameters:
        request.log.debug(
            "Ignoring %s parameters: %s", parameters_type, ", ".join(parameters.keys())
        )
    return sanitized
...
def _sanitize_katib_spec(request, katib_spec):
    ...
    return _sanitize_parameters(request, katib_spec, KATIB_PARAMETER_NAMES, KATIB_DEFAULTS, "Katib")
...
def create_katib_experiment(request, pipeline_id, version_id, pipeline_metadata, output_path):
    ...
    if not katib_spec:
        raise RPCNotFoundError(
            details=("Could not find Katib specification in notebook's metadata"),
            trans_id=request.trans_id,
        )
    ...
    definition_path = os.path.abspath(os.path.join(output_path, f"{katib_name}.katib.yaml"))
    request.log.info("Saving Katib experiment definition at %s", definition_path)
...
def get_experiment(request, experiment, namespace):
    ...
    try:
        exp = k8s_co_client.get_namespaced_custom_object(
            co_group, katibutils.discover_katib_version(), namespace, co_plural, experiment
        )
    except ApiException as e:
        request.log.exception("Failed to get Katib experiment")
        raise RPCUnhandledError(
            message="Failed to get Katib experiment", details=str(e), trans_id=request.trans_id
        )
